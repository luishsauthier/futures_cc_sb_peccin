...
def read_root():
    return {
        ...
        "endpoints": ["/ping", "/futures?roots=CC,SB", "/dolar"]
    }
...
def get_futures(root: str) -> pd.DataFrame:
    ...
    r = session.get(api_url, params=payload, headers=api_headers, timeout=10)
    r.raise_for_status()
...
def fetch_dolar_investing():
    ...
    ts = datetime.fromtimestamp(int(data["lastUpdateTimestamp"]), tz=timezone.utc)
    return price, ts

def fetch_dolar_awesome():
    r = requests.get(AWESOME_API, headers=HEADERS_AWESOME, timeout=10)
    r.raise_for_status()
    usd = r.json()["USD"]
    price = float(usd["bid"])
    ts = datetime.fromisoformat(usd["create_date"].replace(" ", "T")).replace(tzinfo=timezone.utc)
    return price, ts
...
def read_dolar():
    ...
    try:
        ...
        if datetime.now(timezone.utc) - ts <= timedelta(minutes=10):
            return {"source": "investing", "price": price, "datetime": ts.isoformat()}
        print(f"Investing atrasado: {ts.isoformat()}")
    ...
    try:
        ...
        return {"source": "awesomeapi", "price": price, "datetime": ts.isoformat()}
    except Exception as e:
